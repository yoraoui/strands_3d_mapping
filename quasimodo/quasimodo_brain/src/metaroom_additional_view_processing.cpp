...
reglib::Model * getAVMetaroom2(std::string path, bool compute_edges = true, std::string saveVisuals_sp = ""){
    printf("getAVMetaroom2: %s\n",path.c_str());
    if ( ! boost::filesystem::exists( path ) ){return 0;}
    int slash_pos = path.find_last_of("/");
    std::string sweep_folder = path.substr(0, slash_pos) + "/";

    reglib::Model * fullmodel = reglib::Model::loadFast(sweep_folder+"fullmodel_");
    ...
    if(fullmodel->submodels.size() == 0 || fullmodel->submodels.front()->frames.size() < roomData.vIntermediateRoomClouds.size()){
        reglib::Model * sweepmodel = 0;
        if(fullmodel->submodels.size() == 0){
            sweepmodel = new reglib::Model();
        }else{
            sweepmodel = fullmodel->submodels.front();
        }
        ...
        for (size_t i = sweepmodel->frames.size(); i < roomData.vIntermediateRoomClouds.size(); i++){
            ...
            sweepmodel->relativeposes.push_back(sweepmodel->frames.front()->pose.inverse() * sweepmodel->frames.back()->pose);
            sweepmodel->modelmasks.push_back(new reglib::ModelMask(fullmask));
        }
        if(fullmodel->submodels.size() == 0){
            fullmodel->submodels.push_back(sweepmodel);
            fullmodel->submodels_relativeposes.push_back(Eigen::Matrix4d::Identity());
        }
    }
...
//getFrame("44ebfe5c-3a7f-4e3a-9a59-9c51d6a82140");
//exit(0);

//getFrame("de3934eb-6e81-4d22-a382-54973f69b73f");
//getFrame("1eacdb21-2823-43f3-b06a-18b8291b2614");
//getFrame("69a0810c-0b10-49da-b9fa-3e41fb99df6a");
//getFrame("6336ce79-ad3a-4aa5-b349-051de59b5ee4");
//getFrame("e772ea6c-3a5d-4191-84d5-efd123357e84");
//getFrame("4b1152a2-e32c-4c87-a08e-1f2a9c58086e");
//getFrame("5ab1cec8-fed9-42b2-a81d-28cedd395c2b");
//getFrame("91d92a58-3dbd-4842-8b80-2a5df3a915af");
//exit(0);
