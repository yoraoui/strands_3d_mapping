...
void showTest2d(){
    ...
    func->useIRLSreweight                      = false;


    //Eigen::Matrix4d T = getMat(0.25, 0.25, 0.25, 0, 0, 0);
    double angle = 10.0*2.0*M_PI/360.0;
printf("%% angle: %f\n",angle);
    Eigen::Matrix4d T = getMat(angle,angle,angle, 0, 0, 0);
while(true){
    Matrix3Xd inlier_backgrounds = getPoints(1000);
    Matrix3Xd inlier_sampled = getMeasurements(inlier_backgrounds,0.05);
    Matrix3Xd inlier_sampled_T = transform_points(inlier_sampled,T);

    Matrix3Xd outlier_backgrounds = getPoints(1000);
    Matrix3Xd outlier_sampled = getOutlierPoints(outlier_backgrounds);

    cv::Mat img;
    img.create(1500,1500,CV_8UC3);
    unsigned char * data = (unsigned char *)img.data;
    for(int i = 0; i < 1500*1500*3; i++){data[i] = 255;}

    for(int i = 0; i < 10; i++){
        cv::Point p1 = cv::Point(250+1000.0*inlier_backgrounds(0,i),  250+1000.0*inlier_backgrounds(1,i));
        cv::Point p2 = cv::Point(250+1000.0*inlier_sampled(0,i),  250+1000.0*inlier_sampled(1,i));
        cv::line(img,   p1, p2, cv::Scalar(0,255,0), 3);
        cv::circle(img, p1,   10,  cv::Scalar(255,0,255), 3);
        cv::circle(img, p2,   10,  cv::Scalar(255,0,0), 3);
    }
    ...
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    func->reset();
    func->computeModel(inlier_backgrounds-inlier_sampled);
    printf("saveas(gcf,'inliersPlot','epsc')\n");
    cv::namedWindow( "img", cv::WINDOW_AUTOSIZE );
    cv::imshow( "img", img );
    cv::imwrite( "./inliers.png",img);
    cv::waitKey(0);

    int count = 0;
    for(int i = 0; i < 1000; i++){
        if(outlier_sampled(0,i) > -0.25 && outlier_sampled(1,i) > -0.25 && outlier_sampled(0,i) < 1.25 && outlier_sampled(1,i) < 1.25){
            cv::Point p1 = cv::Point(250+1000.0*outlier_backgrounds(0,i),  250+1000.0*outlier_backgrounds(1,i));
            cv::Point p2 = cv::Point(250+1000.0*outlier_sampled(0,i),  250+1000.0*outlier_sampled(1,i));
    ...
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%% Inliers + outliers\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    func->reset();
    func->computeModel(conc(inlier_backgrounds,outlier_backgrounds)-conc(inlier_sampled,outlier_sampled));
    printf("saveas(gcf,'inliersOutliersPlot','epsc')\n");
    cv::namedWindow( "img", cv::WINDOW_AUTOSIZE );
    cv::imshow( "img", img );
    cv::imwrite( "./inliersOutliers.png",img);
    cv::waitKey(0);
    ...
    for(int i = 0; i < 10; i++){
        cv::Point p1 = cv::Point(250+1000.0*inlier_backgrounds(0,i),  250+1000.0*inlier_backgrounds(1,i));
        cv::Point p2 = cv::Point(250+1000.0*inlier_sampled_T(0,i),  250+1000.0*inlier_sampled_T(1,i));
        cv::line(img,   p1, p2, cv::Scalar(0,255,0), 3);
        cv::circle(img, p1,   10,  cv::Scalar(255,0,255), 3);
        cv::circle(img, p2,   10,  cv::Scalar(255,0,0), 3);
    }
    ...
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%% Inliers transformed");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    func->reset();
    func->computeModel(inlier_backgrounds-inlier_sampled_T);
    printf("saveas(gcf,'inliersTransformedPlot','epsc')\n");
    cv::namedWindow( "img", cv::WINDOW_AUTOSIZE );
    cv::imshow( "img", img );
    cv::imwrite( "./inliersTransformed.png",img);
    cv::waitKey(0);
    ...
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%% Inliers transformed + outliers\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    func->reset();
    func->computeModel(conc(inlier_backgrounds,outlier_backgrounds)-conc(inlier_sampled_T,outlier_sampled));
    printf("saveas(gcf,'inliersTransformedOutliersPlot','epsc')\n");
    cv::namedWindow( "img", cv::WINDOW_AUTOSIZE );
    cv::imshow( "img", img );
    cv::imwrite( "./inliersTransformedOutliers.png",img);
    cv::waitKey(0);
...
int main(int argc, char **argv){
	...
	for(int i = 1; i <= 0; i+=1){ translation_transformations.push_back(getMat(0,0,0,double(i*0.01),0,0)); }
    vector<Matrix4d> angle_transformations;
	for(int i = 1; i <= 100; i+=1){ angle_transformations.push_back(getMat(0.01*double(i)*M_PI,0,0,0,0,0)); }
...
//        sfunc->data_per_bin                      5   = 40;
//        sfunc->debugg_print                         = false;//false;
...
////            sprintf(buf,"norange_ppr_%i_%i_%i_%i_ggd",int(100.0*sfunc->startreg),int(100.0*gd->costpen),int(sfunc->data_per_bin),int(1000.0*sfunc->blur));
////        }else{
////            sprintf(buf,"norange_ppr_%i_%i_%i_%i_gausian",int(100.0*sfunc->startreg),int(100.0*gd->costpen),int(sfunc->data_per_bin),int(1000.0*sfunc->blur));
////        }
//        if(sfunc->useIRLSreweight){
//            sprintf(buf,"ppr_%i_ggd",int(100.0*sfunc->startreg));
//        }else{
//            sprintf(buf,"ppr_%i_gausian",int(100.0*sfunc->startreg));
//        }
...
//                }else{
//                    sprintf(buf,"ppr_%i_gausian",i5nt(100.0*sr));
//                }
//                sfunc->name = string(buf);
//                funcs.push_back(sfunc);
//            //}
...
//    }
/*
	for(unsigned int setup = 1; false && setup <= 0b1; setup++){
...
//    test("high", 100, 1000, 10000, 0.01,translation_transformations,angle_transformations,funcs);
    ...
    //            //Matrix3Xd measurements_full	= measurements_outtrueliers;//conc(measurements[i],measurements_outliers);
