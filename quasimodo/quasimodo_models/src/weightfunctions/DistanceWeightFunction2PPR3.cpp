...
void DistanceWeightFunction2PPR3::computeModel(MatrixXd mat){
    ...
    if(!fixed_histogram_size){
        ...
        if(update_size){
            ...
            double next_mind;
            start_time = getTime();
            dist->getMaxdMind(next_maxd,next_mind,0.05);
...
//		Gaussian3 g = Gaussian3(mulval,meanval,stdval);//getModel(stdval,blur_histogram,uniform_bias);
        ...
        for(int i = 0; i < 500; i++){
            ...
            dist->setRegularization(regularization);
            ...
            for(int k = 0; k < histogram_size; k++){
                ...
                new_sum_prob += std::min(maxp , dist->getval(k)/hs) * new_histogram[k];
            }
            ...
            //double change = histogram_size*regularization/(maxd-mind);
//            //printf("DistanceWeightFunction2PPR3::update() %i -> reg %f change %f\n",i,regularization,change);

            //if(change < 0.01*dist->getNoise()){return true;}
