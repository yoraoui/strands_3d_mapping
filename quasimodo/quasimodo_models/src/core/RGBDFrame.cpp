...
RGBDFrame::RGBDFrame(Camera * camera_, cv::Mat rgb_, cv::Mat depth_, double capturetime_, Eigen::Matrix4d pose_, bool compute_normals){
	...
	for(int i = 0; i < 3*width*height; i++){dedata[i] = 0;}
	...
	for(int i = 0; i < width*height; i++){
		cenmsdata[3*i+0] = 0;
		cenmsdata[3*i+1] = cedata[3*i+1]*maxima_dxdata[i];
		cenmsdata[3*i+2] = cedata[3*i+2]*maxima_dydata[i];
	}
	...
	for(unsigned long src_ind = 0; src_ind < nr_data;src_ind++){
		...
		if((dst_w > 0) && (dst_h > 0) && (dst_w < dst_width2) && (dst_h < dst_height2)){
			...
			if(dst_z > 0 && dst_nx != 2){
				...
				ret.push_back(ReprojectionResult (src_ind, dst_ind, angle, residualZ, residualR, residualG, residualB, getNoise(dst_z), 1.0));
			}
		}
	}
	return ret;
